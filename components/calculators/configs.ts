...
export const calcConfigs: Record<string, CalcConfig> = {
  ...
  // ── 15. MER (Marketing Efficiency Ratio) Calculator ──────────────────────
  'mer-calculator': {
    fields: [
      $('total_revenue', 'Total Revenue (All Channels)', 50000, 5000000, 100),
      $('total_ad_spend', 'Total Ad Spend (All Channels)', 10000, 1000000, 100),
    ],
    ready: (v) => v.total_ad_spend > 0 && v.total_revenue > 0,
    formula: 'MER = Total Revenue ÷ Total Ad Spend (across ALL channels)',
    outputs: [
      {
        label: 'Marketing Efficiency Ratio',
        get: (v) => x(v.total_revenue / v.total_ad_spend),
        tone: (v) => {
          const mer = v.total_revenue / v.total_ad_spend;
          if (mer >= 3) return 'good';
          if (mer >= 1.5) return 'warn';
          return 'bad';
        },
        primary: true,
        sublabel: 'Revenue per $1 spent across all marketing',
      },
      {
        label: 'Blended Ad Spend %',
        get: (v) => pct((v.total_ad_spend / v.total_revenue) * 100),
        primary: false,
        sublabel: 'What % of revenue is going back to ads',
      },
    ],
    benchmarks: [
      { label: 'Healthy MER', tone: 'good', range: '3× or above — room to scale' },
      { label: 'Marginal MER', tone: 'warn', range: '1.5–3× — profitable but tight' },
      { label: 'Losing MER', tone: 'bad', range: 'Below 1.5× — marketing losing money' },
    ],
    assumptions: [
      'MER uses total revenue and total ad spend across ALL channels — not single-channel ROAS.',
      'A healthy MER depends on your margins. Use it alongside profit margin to get the full picture.',
      'MER blends organic, email, and paid — use single-channel ROAS separately to diagnose channel performance.',
    ],
    note: (v) => {
      const mer = v.total_revenue / v.total_ad_spend;
      return `Your MER of ${x(mer)} means you earn ${money(v.total_revenue / v.total_ad_spend)} in total revenue for every $1 spent on marketing. Compare this to your single-channel ROAS to see the full attribution picture.`;
    },
  },

  // ── 16. CAC (Customer Acquisition Cost) Calculator ────────────────────────
  'cac-calculator': {
    fields: [
      $('total_spend', 'Total Acquisition Spend', 10000, 1000000, 100),
      numF('new_customers', 'New Customers Acquired', 200, 100000, 1),
      $adv('organic_revenue', 'Organic Channel Revenue (if any)', 500000, 100),
      pctAdv('organic_pct', 'Organic % of New Customer Mix', 100, 1),
    ],
    ready: (v) => v.new_customers > 0 && v.total_spend > 0,
    formula: 'CAC = Total Acquisition Spend ÷ New Customers Acquired',
    outputs: [
      {
        label: 'Customer Acquisition Cost',
        get: (v) => {
          const paidCustomers = v.new_customers * (1 - (v.organic_pct || 0) / 100);
          if (paidCustomers <= 0) return money(0);
          return money(v.total_spend / (paidCustomers || v.new_customers));
        },
        tone: (v) => {
          // Without LTV context we can only flag obviously high relative numbers
          return 'neutral';
        },
        primary: true,
        sublabel: 'What you paid to acquire each new customer',
      },
      {
        label: 'Blended CAC (all customers)',
        get: (v) => money(v.total_spend / v.new_customers),
        primary: false,
        sublabel: 'Spend ÷ all new customers (incl. organic)',
      },
    ],
    benchmarks: [
      { label: 'Healthy CAC:LTV', tone: 'good', range: 'CAC is less than 1/3 of LTV' },
      { label: 'Marginal', tone: 'warn', range: 'CAC is 1/3 to 1/2 of LTV' },
      { label: 'Unsustainable', tone: 'bad', range: 'CAC exceeds LTV — losing on each customer' },
    ],
    assumptions: [
      'True CAC at scale is always higher than platform-reported CPA — it includes creative production, tools, and platform fees.',
      'Pair this with LTV (use the CLV Calculator) to evaluate whether CAC is sustainable.',
      'Organic customer percentage reduces the paid-only CAC calculation.',
    ],
    note: (v) => {
      const cac = v.total_spend / v.new_customers;
      return `Your blended CAC is ${money(cac)}. To evaluate this meaningfully, compare it to your customer LTV. If LTV is ${money(cac * 3)} or more, your acquisition economics are healthy.`;
    },
  },


  // ── 17. Gross Profit Calculator ───────────────────────────────────────────
  'gross-profit-calculator': {
    fields: [
      $('revenue', 'Total Revenue', 10000, 10000000, 100),
      $('cogs', 'Cost of Goods Sold (COGS)', 4000, 5000000, 100),
    ],
    ready: (v) => v.revenue > 0,
    ...
    outputs: [
      {
        label: 'Gross Profit',
        get: (v) => money(v.revenue - v.cogs),
        tone: (v) => {
          const gp = v.revenue - v.cogs;
          if (gp <= 0) return 'bad';
          const margin = (gp / v.revenue) * 100;
          if (margin >= 40) return 'good';
          if (margin >= 20) return 'warn';
          return 'bad';
        },
        primary: true,
        sublabel: 'Revenue minus cost of goods',
      },
      {
        label: 'Gross Margin %',
        get: (v) => {
          if (!v.revenue) return '—';
          return pct(((v.revenue - v.cogs) / v.revenue) * 100);
        },
        primary: false,
      ...
      },
      {
        ...
        get: (v) => {
          if (!v.revenue) return '—';
          return pct((v.cogs / v.revenue) * 100);
        },
        primary: false,
        sublabel: 'What % of revenue goes to product cost',
      },
    ],
    benchmarks: [
      { label: 'Healthy for dropshipping', tone: 'good', range: '40–70% gross margin' },
      { label: 'Tight but workable', tone: 'warn', range: '25–40% gross margin' },
      { label: 'Too thin for paid ads', tone: 'bad', range: 'Below 25% — rarely survives ad costs' },
    ],
    assumptions: [
      'Gross profit covers only revenue minus COGS — it does not subtract advertising, overhead, or SaaS tools.',
      'For dropshipping, COGS = product cost + shipping from supplier + payment processing fees.',
      'Net profit requires subtracting ad spend and operating costs from gross profit.',
    ],
    note: (v) => {
      if (!v.revenue) return '';
      const gp = v.revenue - v.cogs;
      const margin = (gp / v.revenue) * 100;
      return `At a ${pct(margin)} gross margin, you have ${money(gp)} to cover ad spend, returns, tools, and profit. For paid traffic to work, gross margin typically needs to be at least 40%.`;
    },
  },
