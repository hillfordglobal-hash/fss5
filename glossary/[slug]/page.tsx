...
export async function generateMetadata({
  ...
  return {
    ...
    openGraph: {
      ...
      images: [{ url: 'https://firstsalesociety.com/moduleimages/firstsalesociety.png', width: 1200, height: 630, alt: `What is ${term.term}? — First Sale Society` }],
    },
    twitter: {
...
export default async function GlossaryTermPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  // Cross-linked tools (from relatedTools field in glossary data)
  const relatedToolSlugs = term.relatedTools ?? [];
  // Cross-linked glossary terms (cross-category, from relatedTerms field)
  const crossLinkedTerms = (term.relatedTerms ?? [])
    .map((s) => glossaryTerms.find((t) => t.slug === s))
    .filter((t): t is NonNullable<typeof t> => Boolean(t) && t.slug !== term.slug);
    ...
    <>
      ...
      <div className="module-content">
        ...
        <div style={{ fontSize: 12, color: 'var(--text3)', marginTop: 8, marginBottom: 4 }}>
          Last updated June 2026 · <a href="https://firstsalesociety.com" style={{ color: 'var(--text3)' }}>First Sale Society</a>
        </div>
        ...
        {relevantModule && (
          <div style={{ margin: '24px 0', padding: '16px 20px', background: 'var(--surface2)', borderRadius: 'var(--r-md)', border: '1px solid var(--border)' }}>
            <p style={{ margin: 0, fontSize: 14, color: 'var(--text2)' }}>
              <strong style={{ color: 'var(--heading)' }}>See this in the curriculum →</strong>{' '}
        ...
        {/* Related tools — calculators that apply this concept */}
        ...
        {crossLinkedTerms.length > 0 && (
          <div style={{ marginTop: 24 }}>
            <h3>Related concepts</h3>
            <ul>
              {crossLinkedTerms.map((t) => (
                <li key={t.slug}>
                  <Link href={`/glossary/${t.slug}`}>What is {t.term}?</Link>
                </li>
              ))}
        ...
        <div style={{ marginTop: 24, display: 'flex', gap: 12, flexWrap: 'wrap' }}>
          <Link href="/glossary" className="module-nav-btn">← All definitions</Link>
          <Link href="/" className="module-nav-btn primary">Back to the curriculum →</Link>
        </div>
      </div>
    </>
  );
