...
export default async function ModulePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <>
      ...
      <nav aria-label="Breadcrumb" style={{ padding: '10px 24px 0', fontSize: 12.5, color: 'var(--text3)' }}>
        <Link href="/" style={{ color: 'var(--text3)' }}>
          Home
        </Link>
        {' › '}
        <span>{mod.phaseLabel}</span>
        {' › '}
        <span style={{ color: 'var(--text2)' }}>{mod.title}</span>
        <span style={{ marginLeft: 12 }}>
          Module {index + 1} of {courseModules.length}
        </span>
      </nav>

      <div className="tldr-band">
        <strong>TL;DR</strong>
        <span>{mod.metaDescription}</span>
      </div>
